...
namespace ImGui
{
	class ImNotifications
	{
	public:
		static ImNotifications* Inst()
		{
			static ImNotifications inst;
			return &inst;
		}
	...
	{
		ImNotifications::Inst()->notifications.push_back(toast);
	}
	...
	{
		ImNotifications::Inst()->notifications.erase(ImNotifications::Inst()->notifications.begin() + index);
	}
	...
	NOTIFY_INLINE void RenderNotifications()
	{
		...
		for (size_t i = 0; i < ImNotifications::Inst()->notifications.size(); i++)
		{
			auto* current_toast = &ImNotifications::Inst()->notifications[i];
			...
			char window_name[50];
			sprintf(window_name, "##TOAST%d", (int)i);
